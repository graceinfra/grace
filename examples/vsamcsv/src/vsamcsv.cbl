       ...
       01 WS-DETAIL-LINE.
          ...
          05 FILLER            PIC X   VALUE ','.
          05 WS-CSV-SALARY     PIC X(10).
          05 FILLER            PIC X(34) VALUE SPACES. *> Pad to 80
      ...
      *----------------------------------------------------------------
       1000-INITIALIZE.
           ...
           PERFORM 1300-READ-VSAM. *> Initial Read
           .
       1100-OPEN-FILES.
           OPEN INPUT VSAM-IN.
           IF NOT VSAM-OK
      ...
      *----------------------------------------------------------------
       2000-PROCESS-RECORDS.
           ...
           PERFORM 1300-READ-VSAM. *> Read next record
           .
       2100-FORMAT-CSV-LINE.
           MOVE VSAM-EMPL-ID     TO WS-CSV-ID.
           MOVE VSAM-EMPL-NAME   TO WS-CSV-NAME.
           MOVE VSAM-EMPL-SALARY TO WS-CSV-SALARY.
           MOVE WS-DETAIL-LINE   TO CSV-REC.
      *    Alternative using STRING if fields need trimming/precise concat:
      *    INITIALIZE CSV-REC.
      *    STRING VSAM-EMPL-ID     DELIMITED BY SIZE
      *           ','             DELIMITED BY SIZE
      *           VSAM-EMPL-NAME  DELIMITED BY '  ' *> Example trim
      *           ','             DELIMITED BY SIZE
      *           VSAM-EMPL-SALARY DELIMITED BY SIZE
      *      INTO CSV-REC
      *    END-STRING.
           .
       2200-WRITE-CSV-LINE.
